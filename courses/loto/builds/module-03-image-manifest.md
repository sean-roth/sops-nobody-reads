...
**Learner-language rework update (Pass 6):** `module-03.json` was **32 slides**, not 36 — REWORK-2026-07-16.md §7 restructured the module into five movements (cuts, merges, one addition — the notification map, which carries no image by design). "The Critical Ones" slide (headcount/lock-removal zoom-in) was cut, its content folded into the two restoration-list slides' own item text; its image, `s06-headcount.jpg`, was no longer referenced and was quarantined. Every other image assignment was unchanged at that pass.

**Gate B fix pass 2 update (current):** `module-03.json` is now **29 slides**. FIX 1 collapsed the restoration material's three grain sizes into one plus the notification map: the two restoration-list slides were re-split at each notification beat (7 items / 2 items instead of 5/4) so each list slide could directly carry one of the two notification source blocks, and the three per-item expansion slides between them ("Before It Moves," "Re-energize," "Not Optional") were cut, their unique specifics folded into the corresponding list item text. None of those three slides carried an image, so no image was orphaned by this pass. Every other image assignment in the table below is unchanged. Same caveat as above applies with more force now — treat "Slide" numbers here as historical, not current.
