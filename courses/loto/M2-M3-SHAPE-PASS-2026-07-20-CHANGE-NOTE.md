...
M1's sequence connector rule excluded the last item from getting a trailing connector line via `:not(:nth-child(3n))` — which only worked for M1's 6 items because 6 is divisible by 3, so the last item was always also "last in its row." "Before It Restarts" has 7 items, which land 3+3+1 — the 7th sits alone in a third row, and `:nth-child(3n)` alone doesn't exclude it (7 isn't a multiple of 3), so it would have shown a connector line pointing at nothing. Fixed by adding `:not(:last-child)` to the exclusion, in all three players (backported to M1 too, for consistency — harmless there since M1's item 6 was already excluded either way). Screenshot-verified in both modes: the orphan row renders with no dangling line.
